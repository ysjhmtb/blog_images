...
오늘날, 수중 센서는 지상의 센서와 데이터를 공유할 수 없다. 왜냐하면 두 기기는 제한된 영역에서만 동작하는 서로 다른 무선 신호를 사용하기 때문이다. Radio 신호는 공기 중을 움직이다가 물속에서는 바로 사라진다. 음향 신호나 소나는 대부분 수면을 투과하지 못하고 대부분 반사된다. 이런 특성은 통신 불능 외에도 해양탐사나 잠수함과 항공기 사이의 통신 같은 영역에서 여러 문제점을 초래했다.   
...
Adib은 "수면에 대응하는 다양한 치환 방식을 설정할 때마다 레이더 반사는 약간씩 달라질 것이다. 이 작은 각도 변화를 포착함으로서 소나 신호에 대응하는 이 진동들을 포착할 수 있다."라고 밝혔다.  
...
이 문제를 해결하기 위해, 연구진은 정교한 신호처리 알고리즘을 개발했다. 자연 상태의 파도는 1~2 헤르츠 파장으로 발생하거나 매초마다 하나나 두개의 파도가 신호영역을 이동하면서 덮게 된다. 하지만 100에서 200 헤르츠의 소나 진동은 백배 더 빠르다. 이 주파수 차이 때문에, 알고리즘은 느린 파도들을 무시하면서 빠르게 움직이는 파장을 겨냥하게 된다.  
...
연구진은 MIT 캠퍼스의 수영장 두 곳과 물탱크에서 TARF를 500번 시험을 해보았다.   

물탱크에서 레이더는 수면 위 20에서 40 센티미터 거리에 위치시켰고, 소나 전송기는 수면 아래 5에서 70 센티미터에 위치시켰다. 수영장에서는 레이더는 수면 위 약 30센티미터 위에 위치시켰고, 전송기는 수면 아래 약 3.5 미터 지점에 배치했다. 실험을 진행하면서 연구진은 수영하는 사람이 대략 16센티미터 높이의 파도를 만들도록 하였다.   

두 상황에서 TARF는 "Hello! from underwater" 같은 문장을 비롯해 다양한 데이터를 정확하게 해독하는 것이 가능했다. 초당 수백비트의 속도였으며, 수중 통신의 표준 데이터 전송률과 비슷한 속도였다. "심지어 수영하는 사람이 주변을 수영하면서 간섭하는 파장과 물의 흐름을 만들어 내는 동안에도 이들 신호들을 빠르고 정확하게 해독해 낼 수 있었다."   

그러나 16센티미터가 넘는 파도 속에서는 시스템이 신호를 해독해낼 수 없었다.   해결해야 할 다음 문제는 무엇보다 시스템이 더 거친 파도 속에서도 동작하도록 개선하는 것이다. "고요한 날이나 특정한 파도의 간섭 속에서는 시스템이  잘 작동하지만, 실제 상황에서 이용하기 위해서는 모든 날 모든 날씨에서 동작하도록 만들어야 합니다."   

UCSD의 조교수 Aaron Schulman는 다음과 같이 말했다. "TARF는 레이더를 사용하여 대기 중에서 수중의 음향 신호를 수신할 수 있음을 보여준 첫번째 시스템이다. 이 새로운 레이더-음향 기술이 수중 음향학에 의존하는 분야의 연구진(예를 들어 해양 생물학)들을 돕고 과학계가 레이더-음향 기술을 실용적이고 강건하게 만들도록 고무시키기를 기대한다."   

연구진은 또한 그들의 시스템이 궁극적으로 수면을 가로질러서 비행하는 항공기들이 소나 신호를 지나칠 때 지속적으로 신호를 수집하고 해독해내도록 만들기를 희망한다.   

연구는 부분적으로 전미 과학 재단의 지원을 받았다.   
